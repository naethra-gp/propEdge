...
class PropertyListService {
  insert(values) async {
    ...
    values.forEach((val) async {
      ...
      await db.transaction((txn) async {
        await txn.rawInsert(
            'INSERT INTO ${Constants.propertyList} ( PropId, Address, ApplicationNumber, ContactPersonName, ContactPersonNumber, CustomerName, DateOfVisit, InstituteName, LocationName, ColonyName, Priority, Status, SpecialInstruction, SyncStatus ) VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,? ) ',
            [
              propId,
              address,
              applicationNumber,
  ...
  updateLocalStatus(List request) async {
    ...
    try {
      int count = await db.rawUpdate(
          "UPDATE ${Constants.propertyList} SET Status = ?, SyncStatus = ? WHERE PropId = ?",
          request);
      return count;
    } catch (e) {
      return "error -> $e";
  ...
  deleteRecord(List request) async {
    ...
    return await db.rawDelete(
        "DELETE FROM ${Constants.propertyList} WHERE Status =? AND PropId = ?",
        request);
  }

  deleteByPropId(String table, List propId) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawDelete("DELETE FROM $table WHERE PropId = ?", propId);
  }
}
