...
class LiveToLocalInsert {
  ...
  insertAll(String id, data) async {
    ...
    var conditionAndWidthOfApproachRoad   = location['ConditionAndWidthOfApproachRoad'] ?? "";
    var infrastructureOfTheSurroundingArea= location['InfrastructureOfTheSurroundingArea'] ?? "";
    ...
    var proximityFromCivicsAmenities      = location['ProximityFromCivicsAmenities'] ?? "";
    ...
    var progressPerAsPerPolicy    = jsonEncode(calculator['ProgressPerAsPerPolicy']);
    var recommendedPerAsPerPolicy = jsonEncode(calculator['RecommendedPerAsPerPolicy']);
    ...
    await db.transaction((txn) async {
      ...
      await txn.rawInsert("""INSERT INTO ${Constants.customerBankDetails}
          ...
          VALUES ( '$propId', '$bankName', '$contactPersonName', '$contactPersonNumber', '$customerName', '$loanType', '$propertyAddress', '$siteInspectionDate', '$syncStatus') """);

      /// PropertyLocationDetails - INSERT QUERY
      await txn.rawInsert("""INSERT INTO ${Constants.propertyLocation}
        (PropId, City, Colony, PropertyAddressAsPerSite, AddressMatching, LocalMuniciapalBody, NameOfMunicipalBody, SyncStatus )
        VALUES ('$propId', '$city', '$colony', '$propertyAddressAsPerSite', '$addressMatching', '$localMunicipalBody', '$nameOfMunicipalBody', '$syncStatus')""");

      /// LocationDetails - INSERT QUERY
      await txn.rawInsert("""INSERT INTO ${Constants.locationDetails} (PropId, AnyNegativeToTheLocality, ClassOfLocality, ConditionAndWidthOfApproachRoad, InfrastructureOfTheSurroundingArea,
        Latitude, Longitude, Micromarket, NatureOfLocality, NearbyLandmark, NearestBusStop, NearestHospital, NearestMetroStation, NearestRailwayStation, NeighborhoodType, ProximityFromCivicsAmenities,
        ...
        VALUES ( '$propId', '$anyNegativeToTheLocality', '$classOfLocality', '$conditionAndWidthOfApproachRoad', '$infrastructureOfTheSurroundingArea',
         '$latitude', '$longitude', '$microMarket','$natureOfLocality', '$nearbyLandmark', '$nearestBusStop', '$nearestHospital','$nearestMetroStation',
         '$nearestRailwayStation', '$neighborhoodType', '$proximityFromCivicsAmenities', '$siteAccess', '$syncStatus' )""");

      /// OccupancyDetails - INSERT QUERY
      await txn.rawInsert("""INSERT INTO ${Constants.occupancyDetails}
      ...
      await txn.rawInsert("""INSERT INTO ${Constants.feedback}
        (PropId, Amenities, ApproxAgeOfProperty, MaintainanceLevel, SyncStatus )
        VALUES ( '$propId', '$amenities', '$approxAgeOfProperty', '$maintenanceLevel', '$syncStatus')""");

      /// BoundaryDetails - INSERT QUERY
      await txn.rawInsert("""INSERT INTO ${Constants.boundaryDetails}
      ...
      await txn.rawInsert("""INSERT INTO ${Constants.measurementSheet}
        (PropId, SizeType, SheetType, Sheet, SyncStatus ) VALUES ( '$propId', '$sizeType', '$sheetType', '$sheet', '$syncStatus')""");

      /// StageCalculator - INSERT QUERY
      await txn.rawInsert('''INSERT INTO ${Constants.stageCalculator} (PropId, Id, MasterId, Heads, Progress, Recommended, TotalFloor, CompletedFloor, ProgressPer, RecommendedPer, ProgressPerAsPerPolicy, RecommendedPerAsPerPolicy, SyncStatus )
        VALUES ('$propId', '$ids', '$masterId', '$heads', '$progress', '$recommended', '$totalFloor', '$completedFloor', '$progressPer', '$recommendedPer', '$progressPerAsPerPolicy','$recommendedPerAsPerPolicy', '$syncStatus')''');

      /// CriticalComment - INSERT QUERY
      await txn.rawInsert("""INSERT INTO ${Constants.criticalComment}
        (PropId, Comment, SyncStatus) VALUES ('$propId', '$comment', '$syncStatus')""");
