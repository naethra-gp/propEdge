...
class PhotographService {
    ...
    if (values.isNotEmpty) {
      values.forEach((val) async {
        ...
        await db.transaction((txn) async {
          await txn.rawInsert("""
        INSERT INTO ${Constants.photograph}
  ...
  read(String propId) async {
    ...
    return await db.rawQuery(
        "SELECT * FROM ${Constants.photograph} WHERE PropId=$propId AND IsActive='Y'");
  }

  readSync(String propId) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawQuery(
        "SELECT * FROM ${Constants.photograph} WHERE PropId=$propId AND SyncStatus='N'");
  }
  updateSyncStatus(List request) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawUpdate(
        "UPDATE ${Constants.photograph} SET SyncStatus = ? WHERE PropId = ?",
        request);
  }
  updateIsActive(List request) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawUpdate(
        "UPDATE ${Constants.photograph} SET IsActive = ?, IsDeleted = ?, SyncStatus = ? WHERE primaryId = ?",
        request);
  }
  deleteById(String id) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawQuery(
        "DELETE FROM ${Constants.photograph} WHERE primaryId='$id'");
  }
  deleteRecord(List request) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawDelete(
        "DELETE FROM ${Constants.photograph} WHERE PropId = ?", request);
  }
}
