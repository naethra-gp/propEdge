...
class UploadLocationMapService {
    ...
    if (values.isNotEmpty) {
      values.forEach((val) async {
        ...
        await db.transaction((txn) async {
          await txn.rawInsert("""
        INSERT INTO ${Constants.locationMap}
  ...
  read(String propId) async {
    final db = await DatabaseServices.instance.database;
    return await db
        .rawQuery("SELECT * FROM ${Constants.locationMap} WHERE PropId=$propId AND IsActive='Y'");
  }
  readSync(String propId) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawQuery(
        "SELECT * FROM ${Constants.locationMap} WHERE PropId=$propId AND SyncStatus='N'");
  }

  updateSyncStatus(List request) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawUpdate(
        "UPDATE ${Constants.locationMap} SET SyncStatus = ? WHERE PropId = ?",
        request);
  }
  ...
  updateIsActive(List request) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawUpdate(
        "UPDATE ${Constants.locationMap} SET IsActive = ?, IsDeleted = ?, SyncStatus = ? WHERE primaryId = ?",
        request);
  }
  ...
  deleteById(String id) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawQuery(
        "DELETE FROM ${Constants.locationMap} WHERE primaryId='$id'");
  }
  deleteRecord(List request) async {
    final db = await DatabaseServices.instance.database;
    return await db.rawDelete(
        "DELETE FROM ${Constants.locationMap} WHERE PropId = ?", request);
  }
}
